...
public class ShowDocumentExample extends Applet implements ActionListener {
    ...
    public void actionPerformed(ActionEvent act) {
        ...
        if (temp.length() > 6) {
            ...
            if (!temp.substring(0, 7).toUpperCase().equals("HTTP://")) // if not then append it to it.
            {
                temp = "Http://" + temp;
            }
        ...
        if (temp.indexOf(".com") == -1) // So we'll show an error message later
        {
            UrlOnError = true;
        }
        ...
        if (!UrlOnError) // This will do so. Note that the "_blank" part can be 
